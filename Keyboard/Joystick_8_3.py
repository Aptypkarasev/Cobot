...
class Service(object):
    ...
    def loop(self):
        ...
        while True:
            ...
            speeds[1] = 1 * self.joystick.get_hat(0)[0] * self.linear_velocity
            speeds[0] = 1 * self.joystick.get_hat(0)[1] * self.linear_velocity
            if self.cmd.btn0 and not self.cmd.btn1:
                speeds[2] = 1 * -self.linear_velocity
            if self.cmd.btn1 and not self.cmd.btn0:
                speeds[2] = 1 * self.linear_velocity

            #get rotational speed from joystick
            speeds[4] = -1 * self.cmd.axis1 * self.rotational_velocity
            speeds[3] = -1 * self.cmd.axis0 * self.rotational_velocity
...
if __name__ == "__main__":
    HOST = "192.168.8.3"  # The remote host
